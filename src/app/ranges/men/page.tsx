...
const MenRangesPage: React.FC = () => {
    const [searchTerm, setSearchTerm] = useState('');
    const [sortOption, setSortOption] = useState('');
    const [filterCategory, setFilterCategory] = useState('');

    // Fetch men's products from centralized data
    const allMenProducts = getProductsByRange('men');

    // Filter products based on search term and category filter
    let filteredProducts = allMenProducts.filter(product => {
        const matchesSearch = product.name.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesCategory = !filterCategory || product.category === filterCategory;
        return matchesSearch && matchesCategory;
    });

    // Sort products: Topwear first, then Bottomwear, then by selected sort option
    filteredProducts = [...filteredProducts].sort((a, b) => {
        // If no sort option selected, sort by category (topwear first)
        if (!sortOption) {
            if (a.category === 'topwear' && b.category !== 'topwear') return -1;
            if (a.category !== 'topwear' && b.category === 'topwear') return 1;
            return 0; // Maintain original order within same category
        }
        // If sort option is selected, apply that sorting within categories
        else if (sortOption === 'priceLowToHigh') {
            // First sort by category, then by price
            if (a.category === 'topwear' && b.category !== 'topwear') return -1;
            if (a.category !== 'topwear' && b.category === 'topwear') return 1;
        ...
        // Default case - sort by category only
        else {
            if (a.category === 'topwear' && b.category !== 'topwear') return -1;
            if (a.category !== 'topwear' && b.category === 'topwear') return 1;
            return 0;
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                        ...
                        <div className="relative flex items-center justify-center">
                            ...
                            <div className="relative w-full max-w-lg p-5">
                                ...
                                <div className="relative w-full aspect-[3/4] mt-2 overflow-hidden bg-transparent"
                                    ...
                                    <Image
                                        src="/mens/cream-contrast-collar-polo.png"
                                        alt="The Distinguished Gentleman"
                                        layout="fill"
                                        objectFit="cover"
                        ...
                        <div className="flex flex-col justify-center">
                            ...
                            <p className="text-base leading-relaxed text-gray-300 max-w-xl">
                                Our Gentlemen Collection brings together refined tailoring, premium fabrics, and a touch of understated class.
                                Crafted for those who dress with purpose — because style is your signature.
                                Every piece reflects excellence, confidence, and timeless sophistication.
                            </p>
